...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const title = `${concert.title} — ${concert.artist.name} — Ngowamix`;
  const description = `Réservez vos tickets pour ${concert.title} de ${concert.artist.name} à ${concert.venue}, ${concert.city}. ${new Date(concert.date).toLocaleDateString('fr-FR')}.`;

  return {
    title,
    description,
    alternates: { canonical: `/tickets/${id}` },
    openGraph: {
      title,
      description,
      url: `/tickets/${id}`,
      images: concert.poster ? [{ url: concert.poster, width: 1200, height: 630 }] : [{ url: '/og.jpg', width: 1200, height: 630 }],
    },
    twitter: {
      card: 'summary_large_image',
      title,
      description,
      images: concert.poster ? [concert.poster] : ['/og.jpg'],
    },
  };
}
                  ...
                  <div className="flex items-center gap-4">
                    {concert.artist.avatar ? (
                      <SafeImage
                        ...
                        fallback={<div className="h-16 w-16 rounded-full bg-surface-hover flex items-center justify-center"><Music className="h-8 w-8 text-text-muted" /></div>}
                      />
                    ) : (
