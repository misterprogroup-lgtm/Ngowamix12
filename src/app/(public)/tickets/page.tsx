...
async function getConcerts(artistSlug?: string) {
  try {
    const params = new URLSearchParams({ limit: '20' });
    if (artistSlug) params.set('artist', artistSlug);
    const res = await fetch(`${process.env.APP_URL || 'http://localhost:3000'}/api/concerts?${params}`, {
      next: { revalidate: 300 },
    });
...
export default async function TicketsPage({ searchParams }: TicketsPageProps) {
  ...
  const concerts = await getConcerts(artistSlug);
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {concerts.map((concert: {
                ...
                <ConcertCard
                  ...
                  vvipPrice={concert.vvipPrice ? Number(concert.vvipPrice) : null}
