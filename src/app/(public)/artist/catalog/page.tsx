...
export default function ArtistCatalog() {
  ...
  const handleCreateConcert = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setConcertData({ title: '', venue: '', city: '', country: '', date: '', time: '', description: '', totalTickets: '', price: '', vipPrice: '', vvipPrice: '' });
      setPosterFile(null);
      fetchConcerts();
    } catch (error) {
      alert(error instanceof Error ? error.message : 'Erreur lors de la création');
    ...
    <div className="container mx-auto py-8 pb-24">
      ...
      <Modal isOpen={showConcertModal} onClose={() => setShowConcertModal(false)} title="Créer un concert">
        <form onSubmit={handleCreateConcert} className="space-y-4">
          ...
          <Input
            type="number"
            label="Prix VVIP (XOF) - optionnel"
            placeholder="25000"
            value={concertData.vvipPrice}
            onChange={(e) => setConcertData({ ...concertData, vvipPrice: e.target.value })}
            min="0"
          />
