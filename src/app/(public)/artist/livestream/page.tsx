...
import { Plus, Radio, Loader2, Copy, Eye, EyeOff, Trash2, Play, Square } from 'lucide-react';
    ...
    <div className="container mx-auto py-8 pb-24">
      ...
      {showCreate && (
        <div className="mb-6 p-4 rounded-xl bg-surface border border-border space-y-3">
          ...
          <textarea
            ...
            rows={2}
            className="w-full px-3 py-2 rounded-lg bg-surface-hover border border-border text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-primary resize-none"
          />
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {streams.map((stream) => (
            <div key={stream.id} className="rounded-xl bg-surface border border-border p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-4 mt-2 text-xs text-text-muted">
                    ...
                    <span>{new Date(stream.createdAt).toLocaleDateString('fr-FR')}</span>
                  </div>
                  {stream.streamKey && (
                    <div className="mt-2 flex items-center gap-2">
                      <code className="text-[11px] bg-surface-hover px-2 py-1 rounded font-mono text-text-muted truncate max-w-[250px]">{stream.streamKey}</code>
                      <button onClick={() => copyKey(stream.streamKey)} className="text-text-muted hover:text-primary transition-colors" title="Copier la clé">
                        {copiedKey === stream.streamKey ? <Eye className="h-3.5 w-3.5 text-success" /> : <Copy className="h-3.5 w-3.5" />}
                      </button>
                    </div>
                  )}
                </div>
