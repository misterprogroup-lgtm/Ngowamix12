...
export async function generateMetadata({ params }: AlbumPageProps): Promise<Metadata> {
  ...
  const description = album.description || `${album.type === 'SINGLE' ? 'Single' : album.type === 'EP' ? 'EP' : 'Album'} de ${album.artist.name}`;
  return {
    title,
    description,
    ...
    openGraph: {
      ...
      type: 'music.album',
      url: `/album/${id}`,
      images: album.coverImage ? [{ url: album.coverImage, width: 1200, height: 1200 }] : [],
    },
    twitter: {
