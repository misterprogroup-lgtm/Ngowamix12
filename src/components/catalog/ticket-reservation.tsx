...
export function TicketReservation({ concert }: TicketReservationProps) {
  ...
  const maxQty = ticketType === 'VIP'
    ...
    : ticketType === 'VVIP'
    ? (concert.vvipAvailableTickets || 0)
    ...
    try {
      ...
      const res = await fetch('/api/payment/init', {
        ...
        body: JSON.stringify({
          ...
          description: `${quantity > 1 ? `${quantity}x ` : ''}Billet ${ticketType === 'VVIP' ? 'VVIP' : ticketType === 'VIP' ? 'VIP' : 'Standard'} - ${concert.title}`,
    ...
    <div className="rounded-xl border border-border bg-surface p-6 sticky top-24 space-y-4">
      ...
      <div className="space-y-3">
        ...
        {concert.vvipPrice && (
          <button
            type="button"
            onClick={() => { setTicketType('VVIP'); setQuantity(1); }}
            disabled={concert.vvipAvailableTickets === 0}
            className={`w-full flex justify-between items-center p-3 rounded-lg border text-left transition-colors ${
              ticketType === 'VVIP'
                ? 'border-primary bg-primary/5'
                : 'border-border hover:border-text-muted'
            } ${concert.vvipAvailableTickets === 0 ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
          >
            <div>
              <p className="font-medium text-text-primary flex items-center gap-1">
              ...
              <p className="text-sm text-text-muted">
                {concert.vvipAvailableTickets === 0 ? 'Complet' : `${concert.vvipAvailableTickets} places`}
              </p>
            </div>
            <p className="text-lg font-bold text-accent">
              {concert.vvipPrice.toLocaleString('fr-FR')} F
            </p>
          </button>
        )}
      </div>
        ...
        <>
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-text-primary placeholder:text-text-muted focus:outline-hidden focus:ring-2 focus:ring-primary"
            />
          </div>
        </>
      )}
